...
const fetchAddresses = async () => {
  try {
    ...
    // The API returned a valid JSON response
    if (data.status === false) {
      // Backend explicitly says "Address not found" or similar
      alert(data.message || 'Address not found – please add one');
      
      // jump to the "add address" form
      setStep('form');
    ...
    // Network error, server 4xx/5xx, or malformed response
    // const msg =
    ...
    // again, open the "add address" form
    setStep('form');
    setShowAddressModal(true);
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          setForm((f) => ({
            ...f,
            street: a.road + ', ' + a.quarter + ', ' + a.suburb,
  ...
  const handleAddAddress = async () => {
    ...
    try {
      ...
      console.error('Address ADD error:', e);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (needRetry) {
        ...
        await refresh();
        const { data: second } = await doCheckout();
        data = second;
      }

      if (data?.status === true) {
        console.log(data.status, 'checkout status...');
    ...
    <div className="max-w-5xl mx-auto py-10 px-4">
      ...
      {cartItems.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-6">
            {cartItems.map((item) => (
              <div key={item.cartid} className="grid grid-cols-12 items-center">
                ...
                <div className="col-span-3 flex items-center justify-center gap-4">
                  <QtyBox
                    value={item.qty}
                    onDec={() => dec(item.cartid, item.id)}
                    onInc={() => inc(item.cartid, item.id)}
                  />
                  <button
                    onClick={() => remove(item.cartid, item.id)}
                    className="underline text-[#6d5a52] text-sm"
                  >
                    Remove
                  </button>
                </div>

                <div className="col-span-2 text-right text-[#2A3443] font-semibold text-lg">
      ...
      {showAddressModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-[#fdf8f5] w-full max-w-5xl rounded-3xl shadow-2xl p-10 overflow-hidden">
            <CloseBtn onClick={handleCancel} />

            {/* STEP: FORM */}
            {step === 'form' && (
              <>
                <h2 className="text-3xl font-semibold text-[#6d5a52] mb-6">
                ...
                <div className="grid grid-cols-3 gap-6 text-[#6d5a52]">
                  {[
                    ['doorno','Door No'], ['house','House'], ['street','Street'],
                    ['city','City'], ['pincode','Pincode'], ['district','District'],
                    ['state','State'], ['country','Contry'], ['company','Company'],
                    ['gst','Gst'], ['type','Type'],
                  ].map(([k,l]) => (
            ...
            {step === 'select' && (
              <>
                <h2 className="text-3xl font-semibold text-[#6d5a52] mb-8">
                  Choose Address
                </h2>
                      ...
                      {addresses.map((a) => (
                        <label
                          ...
                          className={`block border rounded-2xl p-4 cursor-pointer text-sm leading-snug ${
                            shippingId === a.id
                              ? 'border-[#b49d91] bg-white'
                              : 'border-[#d7c6bf] bg-[#f6ebe6]'
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <h4 className="text-xl font-semibold text-[#6d5a52]">
                    Delivery Method
                  </h4>
                  <select
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <label
                      className="block border border-[#b49d91] rounded-2xl p-4 text-sm leading-snug text-[#6d5a52] mb-8"
                    >
                      <div className="flex items-start gap-3">
                        <input
                          type="radio"
